<footer  class="pt-4 my-5 pt-md-5 border-top">
    <div class="col-12 col-md">
        <small class="d-block mb-3 text-muted">:: © 2023 : PhotoForYou ::</small>
    </div>
    <div class = "row">
            <div class="col-4 col-md">
                <h5>Lien</h5>
                <ul class="list-unstyled text-small">   
                    <li><a class="text-muted" href="#">Nos promotions</a></li>
                    <li><a class="text-muted" href="#">Les maçons</a></li>
                    <li><a class="text-muted" href="#">Notre charte qualité</a></li>
                </ul>
            </div>

            <div  class="col-4 col-md" > 
            <h5>Aides</h5>
                <ul class="list-unstyled text-small">   
                    <li><a class="text-muted" href="#">Nous contacter</a></li>
                    <li><a class="text-muted" href="#">Tuto</a></li>
                    <li><a class="text-muted" href="#">Tutos Aide en ligne</a></li>
                </ul>
            </div>

            <div  class="col-4 col-md">
                <h5>Nous connaitre</h5>
                <ul class="list-unstyled text-small">   
                    <li><a class="text-muted" href="#">Adresse</a></li>
                    <li><a class="text-muted" href="#">Contact</a></li>
                    
                </ul>
            </div>
        </div>
</footer>